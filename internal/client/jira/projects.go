...
func (c *JiraClient) GetProject(input GetProjectInput) (types.MapOutput, error) {
	...
	err := c.executeRequest(http.MethodGet, []string{"rest", "api", "2", "project", input.ProjectKey}, nil, nil, &project, utils.AcceptJSON)
	if err != nil {
		return nil, err
	}
...
func (c *JiraClient) GetAllProjects(input GetAllProjectsInput) ([]types.MapOutput, error) {
	...
	err := c.executeRequest(http.MethodGet, []string{"rest", "api", "2", "project"}, queryParams, nil, &projects, utils.AcceptJSON)
	if err != nil {
		return nil, err
	}
